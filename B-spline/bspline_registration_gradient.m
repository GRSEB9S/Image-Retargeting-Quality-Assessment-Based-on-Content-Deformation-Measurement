...
% of two images / volumes.该函数将在两个图像/体积的b样条非刚性配准之后计算配准误差值和梯度。
%
% [error, errorgrad]=bspline_registration_gradient(grid,sizes,Spacing,I1,I2,options)
%
% inputs,
%   grid: b-spline control grid created by make_init_grid.m, (and reshaped to one long vector.)        
%   sizes: sizes need tot reshape the grid to matrix format.
%   Spacing: The spacing in x,y (and z) direction of the b-spline grid  knots
%         
...
I_init=bspline_transform(O_grid,I1,Spacing);  %调
   
% Calculate the current registration error
t=image_difference(I_init,I2,type);  %调
O_error=t;
...
if(penaltypercentage>0)
    ...
    if ( nargout > 1 )
        ...
        [SO_error, SO_grad]=penalties_smoothness(O_grid,size(I1)); %调
    else
