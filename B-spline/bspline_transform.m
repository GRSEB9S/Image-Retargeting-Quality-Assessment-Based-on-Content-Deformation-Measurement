function [I,Tx,Ty,Tmap,Tz]=bspline_transform(O,I,Spacing)
...
% Check if spacing has integer values 间距是否是整数  %floor 是近似负无穷大
if(sum(Spacing-floor(Spacing))>0), error('Spacing must be a integer'); end

if(ndims(I)==2)         
    if(~isa(I,'double')), I=im2double(I); end
    if(nargout > 1 )
        [I,Tx,Ty,Tmap]=bspline_transform_2d_double(O(:,:,1),O(:,:,2),I,Spacing(1),Spacing(2));
    else
