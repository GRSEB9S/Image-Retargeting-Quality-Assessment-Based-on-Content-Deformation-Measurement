...
// Convert 2D/3D matrix index to 1D index定义的函数mindex2
...
unsigned __stdcall  transformvolume(double **Args)
{
    ...
    // Location of pixel which will be come the current pixel像素的位置将来自当前像素
    double Tlocalx;
    double Tlocaly;
    
    // Linear interpolation variables线性差值变量
    int xBas[4], yBas[4];
    double perc[4];
    double xCom, yCom;
    ...
    // Variables to store 1D index存储1D索引的变量
    int indexI;
    
    // Multiple threads, one does the odd the other even indexes多线程 一个奇数其余是偶数
    int offset;
    //int start;
    //int end;
    ...
    // Loop through all image pixel coordinates循环遍历所有图像像素坐标
    for (y=offset; y<Isize[1]; y=y+2)
    {
        for (x=0; x<Isize[0]; x++)
        {
            ...
            // Linear interpolation constants (percentages)线性插值常数（百分比）
