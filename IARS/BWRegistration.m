...
%   Detailed explanation goes here
    ...
    SIFTflowpara.nlevels= LevelAdaptiveSetting(img_org); % update the hierarchical levels调用LevelAdaptiveSetting
    SIFT_COE = 1; COLOR_COE = 100; POS_COE = 10; % standard setting

    original_im_d=im2double(img_org);
    sift_org = mexDenseSIFT(original_im_d,cellsize,gridspacing);  %原图调用
    % modify the feature
    colorTransform = makecform('srgb2lab');   
    %makecform - 创建颜色变换结构,该函数创建定义的颜色变换结构C由类型指定的颜色空间转换。 srgb to lab颜色空间
    lab_org = applycform(original_im_d, colorTransform);  %把上述颜色空间转换应用于此图像
    [yy_org, xx_org] = meshgrid(1:size(original_im_d,2), 1:size(original_im_d,1));  %[X,Y] = meshgrid(xgv,ygv)
    ...
    sift_ret = mexDenseSIFT(ret_im,cellsize,gridspacing);  %重构图调用
    % modify the feature
