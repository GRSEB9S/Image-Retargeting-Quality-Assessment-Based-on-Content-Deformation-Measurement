...
void BPFlow::ComputeDataTerm()
{
	...
	HistMax=0;
	// my modifications
	// add the color components and the position components
	// 1~128 is the SIFT, 129~131 are the color components and 131~132 are the position components
	// 133~135 are the SIFT, color and position coefficients respectively
	int N_Color = 3;
	int N_Pos = 2;
	int N_Coef = 3;
	int COEF_SIFT = 255;
	int COEF_COLOR = 255;
	int COEF_POS = 255;
	int n_channels_sift = 128;
	//--------------------------------------------------------------------------------------------------
	// step 1. the first sweep to compute the data term for the visible matches第一次扫描计算可见匹配的数据项
	//--------------------------------------------------------------------------------------------------
		...
		for(ptrdiff_t j=0;j<Width;j++)		// index over x
		{
			...
			for(ptrdiff_t k=-pWinSize[1][index];k<=pWinSize[1][index];k++)  // index over y
				for(ptrdiff_t l=-pWinSize[0][index];l<=pWinSize[0][index];l++)  // index over x
				{
					...
					T_message foo=0;
					T_message foo_color=0;
					T_message foo_pos=0;
					for(int n=0;n<n_channels_sift;n++)
						foo+=abs(pIm1[index*nChannels+n]-pIm2[index2*nChannels+n]); // L1 norm
					for(int n=(n_channels_sift);n<(n_channels_sift+N_Color);n++)
						foo_color+=abs(pIm1[index*nChannels+n]-pIm2[index2*nChannels+n]); // L1 norm
					for(int n=(n_channels_sift+N_Color);n<(n_channels_sift+N_Color+N_Pos);n++)
						foo_pos+=abs(pIm1[index*nChannels+n]-pIm2[index2*nChannels+n]); // L1 norm
					int n = (n_channels_sift+N_Color+N_Pos);
					COEF_SIFT = pIm1[index*nChannels+n];
					n++;
					COEF_COLOR = pIm1[index*nChannels+n];
					n++;
					COEF_POS = pIm1[index*nChannels+n];
					foo = foo*COEF_SIFT+foo_color/N_Color*COEF_COLOR+foo_pos/N_Pos*COEF_POS;
