...
% To run the code prepare the RetargetMe dataset and correct the root path
clear all; 
%% path and other initial information
PATH_ROOT = 'E:\ICDM_data\data\'; % the path direct to the MIT dataset
...
for set_num =20:SET_NUM
    ...
    path2=['E:\ICDM_data\ABdata\'  PATH_NAME{set_num} '\'];
    file = dir([path,'*.png']);   
    ...
    for op_num = 1:OP_NUM
        ...
        im_ret_rg{op_num} = imread([path2 file(retarget_name(op_num)).name]);
    end
    smap =  imread(['E:\ICDM_data\IARS\MIT_smap\' PATH_NAME{set_num} '_smap.png']);
...
for set_num = 20:SET_NUM
    ...
    for op_num = 1:OP_NUM      
        ...
        All_delet_ratio(set_num,op_num)=delet_ratio;
         ...
         img_rbd{set_num} = imread(['E:\ICDM_data\data\rbd\' PATH_NAME{set_num} '_rgb.png']); %对应原图RBD   
        ...
        Spacing=[16 16];  
        [O_trans]=make_init_grid(Spacing,size(I1));  %调 % Make the Initial b-spline registration grid
        ...
        Icor=bspline_transform(O_trans,I1,Spacing); %Transform the input image with the found optimal grid
...
Varsum_b=varsal(20,SET_NUM,OP_NUM,Tx_all,Ty_all,img_rbd);
%%
%IARS evaluation
...
%%
%score
final_score= score_IARS(set_num,:)-(cell2mat(Varsum_b(set_num,:))+All_delet_ratio(set_num,:)*0.01)*0.04+ratio_bi(set_num,:)*0.02;
% KRCC evaluation
car_KRCC= getKLCorr(subj_data(set_num,:),final_score);
disp(['the correlation coefficient of car1 -- KRCC='  num2str(car_KRCC)]);
%MIT dataset meanKRCC
load('globaldef.mat')
load('IARS.mat')
load('oc_delet.mat')
load('score_ff.mat')
f=score_ARS-0.004*(Varsum_b+0.01*All_delet_ratio)+score_f*0.02;
for set_num = 1:37
     KRCC(set_num) = getKLCorr(subj_data(set_num,:), f(set_num,:));  
end
